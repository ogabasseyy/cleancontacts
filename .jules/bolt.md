...
## 2026-10-18 - Replacing chained map and filter with single-pass HashMap/ArrayList loops
**Learning:** Using chained functional operations like `.map { ... }` followed by `.filter { ... }` or `.associate { ... }` on large collections creates unnecessary intermediate collections. In high-frequency paths like contact scanning or refreshing, this forces the garbage collector to clean up large temporary lists. Also, avoid computing unnecessary operations into unused variables.
**Action:** Always combine multi-pass mappings, filters, and associations into a single data structure iteration (e.g. `HashMap(size)` or `ArrayList(size)`) when transforming large collections in performance-critical paths (e.g., repository syncs). Pre-allocate the structure to the expected maximum size to prevent dynamic resizing overhead.
## 2025-02-12 - Eliminate Multi-Pass Collection Allocations in Contact Repository
**Learning:** During large-scale SQL scans or contact streaming processing (e.g. `batchContacts.map {}.addAll()`, then `allEntities.filter {}`), using chained functional collection operations creates excessive `ArrayList` allocations and high Garbage Collection pressure. This specific codebase handles arrays of thousands of contacts during its sync process.
**Action:** Replace multi-pass `.map`, `.addAll`, and `.filter` streams with a manual, single-pass indexed `for` loop `for (i in batchContacts.indices)`. Handle validations (`if (isValid)`) within the loop and add directly to the final `allEntities` list to prevent intermediate memory allocations.
...
## 2026-06-18 - Replacing functional chains with single-pass iterator loops
**Learning:** In `CrossAccountViewModel.kt`, processing nested structures using chained functional operations like `_crossAccountContacts.value.flatMap { it.accounts }.filter { ... }` creates unnecessary intermediate lists and traverses the collection multiple times, forcing the garbage collector to work harder.
**Action:** Replace multi-pass functional chains (.flatMap.filter) with a single-pass nested loop utilizing standard iterators (`for (contact in crossAccounts)`) combined with `ArrayList` and `HashSet`. Always use `HashSet.add(key)` to combine existence check and insertion into a single O(1) operation.
