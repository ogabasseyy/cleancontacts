...
## 2026-05-07 - Avoiding Multi-pass Functional Iterators in Hot Loops\n**Learning:** In high-frequency collection processing loops (like ), chained operations such as `.map { it.trim() }.filter { it.isNotBlank() }` internally create new ArrayList instances and generate  objects on each call. This significantly increases Garbage Collection pressure and reduces throughput. Additionally, relying on `.forEach` inside such loops introduces slight lambda invocation overhead without the ability to cleanly  or short-circuit.\n**Action:** Replace  and  chains in high-frequency list processing with explicit index-based `for` loops (e.g., `for (j in contact.numbers.indices)`) and direct primitive logic. Always document performance-driven departures from idiomatic Kotlin with inline comments.
## 2026-10-18 - Avoiding Multi-pass Functional Iterators in Hot Loops
**Learning:** In high-frequency collection processing loops (like `AndroidMergeUtils.buildMergedContact`), chained operations such as `.map { it.trim() }.filter { it.isNotBlank() }` internally create new ArrayList instances and generate `Iterator` objects on each call. This significantly increases Garbage Collection pressure and reduces throughput. Additionally, relying on `.forEach` inside such loops introduces slight lambda invocation overhead without the ability to cleanly `break` or short-circuit.
**Action:** Replace `.map` and `.filter` chains in high-frequency list processing with explicit loops (e.g., `for (contact in contacts)`) and direct primitive logic. Always document performance-driven departures from idiomatic Kotlin with inline comments.
