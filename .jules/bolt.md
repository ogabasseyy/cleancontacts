...
## 2026-10-18 - Avoid Hidden O(N^2) in List Intersections and functional overhead
**Learning:** Using chained `.flatMap { ... }.filter { ... }` operations creates intermediate collections and unnecessary iterator objects. Additionally, checking `set.contains()` followed by `set.add()` on uniqueness filtering incurs double hash lookups.
**Action:** Replace `flatMap`/`filter` functional chains with single-pass standard iterator loop utilizing an `ArrayList` and `HashSet`. Use `HashSet.add()`'s boolean return value directly as the condition to insert into the result array to save the double lookup. Avoid indexed `for` loops on generic collections.
