...
## 2026-10-18 - Avoid Hidden Collections in Functional Chains
**Learning:** Using chained functional operations like `.flatMap { ... }.filter { ... }` on large collections creates unnecessary intermediate collections and allocations. In high-frequency operations, this forces the garbage collector to clean up large temporary objects.
**Action:** Always combine multi-pass mappings and filters into a single data structure iteration (e.g., `ArrayList` + `HashSet`) when transforming large collections in performance-critical paths. Combining existence check and insertion using `HashSet.add()` reduces redundant operations to O(1).
