...
## 2026-10-18 - Replacing chained flatMap and filter with single-pass loops
**Learning:** Chaining functional operations like `.flatMap { ... }.filter { ... }` on large collections forces the creation of multiple intermediate `ArrayList` objects. Each step in the chain allocates memory that the garbage collector must subsequently clean up.
**Action:** Replace `flatMap` and `filter` chains with pre-allocated imperative structures (like `ArrayList` and `HashSet`). Furthermore, utilize `HashSet.add(key)`'s boolean return value to perform check-and-insert in a single operation, eliminating double lookups.
