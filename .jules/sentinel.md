...
## 2026-04-07 - Prevent Attribute-based XSS in Custom Escape\n**Vulnerability:** The `escapeHtml` function in `landing-page/api/feedback.ts` failed to escape single quotes (`'`), leaving the application vulnerable to attribute-based Cross-Site Scripting (XSS) if user input was embedded within single-quoted HTML attributes.\n**Learning:** When writing custom HTML escapers instead of using standard libraries, it is easy to miss edge cases. Single quotes are just as dangerous as double quotes in HTML contexts.\n**Prevention:** Always escape single quotes (`'` to `&#39;`) alongside other standard characters (`<`, `>`, `&`, `"`) when implementing manual HTML sanitization.
