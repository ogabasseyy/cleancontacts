...
class ExportUseCase @Inject constructor(
    ...
    suspend operator fun invoke(types: Set<ContactType>): Result<Uri> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun exportContactList(contacts: List<Contact>, groupName: String): Result<Uri> = withContext(Dispatchers.IO) {
        try {
            val sanitizedName = groupName.replace(Regex("[^a-zA-Z0-9]"), "_").lowercase()
            val fileName = "contacts_${sanitizedName}_${System.currentTimeMillis()}.csv"
    ...
    private suspend fun generateAndSaveCsv(contacts: List<Contact>, fileName: String): Result<Uri> {
