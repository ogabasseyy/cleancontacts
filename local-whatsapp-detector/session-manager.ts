...
export class SessionManager {
  ...
  /**
   * Validates userId to prevent path traversal and ensure consistent format.
   * Only allows alphanumeric characters, hyphens, and underscores.
   */
  private validateUserId(userId: string): void {
    if (!/^[a-zA-Z0-9_-]+$/.test(userId)) {
      throw new Error('Invalid userId: Only alphanumeric characters, hyphens and underscores are allowed')
    }
  }
