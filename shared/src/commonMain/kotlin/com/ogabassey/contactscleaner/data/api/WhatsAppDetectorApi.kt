...
class WhatsAppDetectorApi(
    ...
    private companion object {
        ...
        private const val MAX_PHONE_NUMBER_LENGTH = 50
    ...
    suspend fun getSessionStatus(userId: String): SessionStatus {
        if (userId.length > MAX_USER_ID_LENGTH) return SessionStatus(connected = false, error = "Invalid user ID")
        return try {
    ...
    suspend fun requestPairingCode(userId: String, phoneNumber: String): PairingResponse {
        if (userId.length > MAX_USER_ID_LENGTH) return PairingResponse(success = false, error = "Invalid user ID")
        if (phoneNumber.length > MAX_PHONE_NUMBER_LENGTH) return PairingResponse(success = false, error = "Invalid phone number")
    ...
    suspend fun disconnect(userId: String): DisconnectResponse {
        if (userId.length > MAX_USER_ID_LENGTH) return DisconnectResponse(success = false, error = "Invalid user ID")
        return try {
    ...
    suspend fun checkNumbers(userId: String, numbers: List<String>): CheckNumbersResponse {
        if (userId.length > MAX_USER_ID_LENGTH) return CheckNumbersResponse(success = false, results = emptyList(), error = "Invalid user ID")
        if (numbers.size > MAX_BATCH_SIZE) return CheckNumbersResponse(success = false, results = emptyList(), error = "Batch size exceeds limit")
    ...
    suspend fun checkNumbersBatch(
        ...
        if (userId.length > MAX_USER_ID_LENGTH) return BatchCheckResponse(success = false, total = 0, checked = 0, whatsappCount = 0, results = emptyList(), error = "Invalid user ID")
        if (numbers.size > MAX_BATCH_SIZE) return BatchCheckResponse(success = false, total = 0, checked = 0, whatsappCount = 0, results = emptyList(), error = "Batch size exceeds limit")
